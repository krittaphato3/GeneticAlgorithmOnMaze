...
public class GeneticSolverII extends BaseGeneticSolver {
    ...
    public GeneticSolverII(Maze maze, int popSize, int generation, int geneLength,
        ...
        this.userPop = popSize;
        this.userGen = generation;
        this.userLen = geneLength;
        this.userMutation = adaptiveRates[0];
        this.userElitism = nElitism;
    }
        ...
        for (int i = 0; i < POPULATION_SIZE; i++) {
            ...
            for (int j = 0; j < GENOME_LENGTH; j++) {
                if (guided) {
                    double r = rand.nextDouble();
                    if (r < 0.4) {
                        genes[offset + j] = (byte) bias1;
                    } else if (r < 0.8) {
                        genes[offset + j] = (byte) bias2;
                    } else {
